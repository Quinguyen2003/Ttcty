  ...
  const handleSave = async (event) => {
    ...
    try {
      if (isAddMode) {
        ...
        // Cập nhật bài viết
        await axios.put(`http://localhost:5000/api/news/${selectedPost.id}`, data, {
          headers: { Authorization: `Bearer ${token}` },
        });
        Swal.fire({
          title: 'Success',
                ...
                <tr key={post.id}>
                  ...
                  <td>
                    <div className="action-buttons">
                      <button className="edit" onClick={() => openEditModal(post)}>
          ...
          <div className="modal-content">
            ...
            <h2>{isAddMode ? "Add New Post" : "Edit Post"}</h2>
              <div className="edit-info">
                <label>Title</label>
                <input type="text" name="title" defaultValue={selectedPost?.title || ""} />
              </div>
              <div className="edit-info">
                <label>Cover Image</label>
                <input type="text" name="cover" defaultValue={selectedPost?.cover || ""} />
              </div>
              <div className="edit-info">
                <label>Category</label>
                <input type="number" name="category_id" defaultValue={selectedPost?.category_id || ""} />
              </div>
              <div className="edit-info">
                <label>Tags</label>
                <input type="number" name="tags_id" defaultValue={selectedPost?.tags_id || ""} />
              </div>
              <div className="edit-info">
                <label>Description</label>
                <input type="text" name="desc" defaultValue={selectedPost?.desc || ""}></input>
              </div>
              <div className="edit-info">
                <label>Author</label>
              ...
              <div className="edit-info">
                <label>Date</label>
                <input type="date" name="date" defaultValue={selectedPost?.date || ""} />
              </div>
